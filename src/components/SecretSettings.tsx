  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content glass-panel" onClick={e => e.stopPropagation()}>
        ...
        <h2 style={{ fontFamily: 'Jua', marginBottom: '20px', color: '#57606f' }}>🔒 선생님 모드</h2>
        <p style={{ marginBottom: '20px', fontSize: '0.9rem', color: '#747d8c' }}>
        ...
        <div style={{ minHeight: '100px', maxHeight: '200px', overflowY: 'auto', marginBottom: '20px', background: 'rgba(0,0,0,0.05)', padding: '10px', borderRadius: '12px' }}>
          {queue.length === 0 ? (
            <p style={{ color: '#888', textAlign: 'center' }}>대기열이 비어있습니다.</p>
          ) : (
            queue.map((name, index) => (
              <div key={name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', background: 'white', borderRadius: '8px', marginBottom: '5px', boxShadow: '0 2px 4px rgba(0,0,0,0.05)' }}>
